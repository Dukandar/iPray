...
class InviteMembersViewController: UIViewController  {
    ...
    override func viewDidLoad()
    {
        ...
        if groupName != ""
        {
            //headerName.text = groupName.uppercased() + "'S GROUP MEMBERS"
            headerName.text = groupName.uppercased() + " \(ConstantMessage.kMEMBERS)"
        }
    ...
    func inviteToGroupWithfriends()
    {
        ...
        if  seletedArrayList.count == 0
        {
            ...
            ServiceUtility.showMessageHudWithMessage("\(ConstantMessage.kAlreadyInvited)" as NSString, delay: ConstantMessage.kDelay)
            return
        }
        ...
        for i in 0..<seletedArrayList.count
        {
            ...
            {
                let id = tempdic.object(forKey: ConstantMessage.kIPrayUser) as! String
                if newUserIdList != ""
                {
                    newUserIdList = newUserIdList + ","
                }else
                {
                    newUserIdList = newUserIdList + id
                }
            }
        }
        inviteToGroupWebService(userIdList : newUserIdList , mobileList : newMobileList )
    }
}

// MARK: - TextField Delegate
extension InviteMembersViewController : UITextFieldDelegate
{
...
extension InviteMembersViewController: contactfetchSuccesFully
{
    func reloadview()
    {
...
extension InviteMembersViewController: UITableViewDataSource, UITableViewDelegate
{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        if splitView.selectedSegmentIndex == 1
        {
            ...
            let alert = UIAlertController(title: ConstantMessage.kRemove, message: ConstantMessage.kWantToRemove1, preferredStyle: UIAlertController.Style.alert)
...
// MARK: - Web Service
extension InviteMembersViewController
{
    // as per PHP developer we send both user id and mobile id
    func inviteToGroupWebService(userIdList : String , mobileList : String )
    {
        self.view.endEditing(true)
        ...
        perameter.setValue("0", forKey: ConstantMessage.kIsAdmin)
    ...
    func removeFromGroupWebService(index : Int)
    {
        ...
        perameter.setValue(UserManager.shareManger.userID!, forKey: ConstantMessage.kUserID)
        perameter.setValue(ShareGroupID, forKey: ConstantMessage.kGroupID)
        perameter.setValue("0", forKey: ConstantMessage.kAsAdmin)
